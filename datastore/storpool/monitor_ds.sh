    ...
    if [[ -d "${SP_DS_TMP}" ]]; then
        ...
        if [[ ${_ret} -eq 0 ]]; then
            ...
            eval "${SP_CMD_VOLUME_SPACE}" 2>"${SP_DS_TMP}/ERROR-volume-eval" |\
              jq -r ".data[]|[.tags[\"img\"],.storedSize,.spaceUsed]|@csv" 2>"${SP_DS_TMP}/ERROR-volume" |\
            ...
            eval "${SP_CMD_SNAPSHOT_SPACE}" 2>"${SP_DS_TMP}/ERROR-snapshot-eval" |\
              jq -r --arg img img --arg snap snap \
                '.data[]|"\(.tags[$img])-\(.tags[$snap]),\(.storedSize),\(.spaceUsed)"' \
                2>"${SP_DS_TMP}/ERROR-snapshot" |\
                sort >>"${SP_DS_TMP}/sizes"
        fi
    fi

    SP_DS_SIZES="$(bash "${SP_MONITOR_DS}" system "${ds:-0}" 2>/dev/null)"
