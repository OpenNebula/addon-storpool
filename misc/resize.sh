...
done 5< <(onevm show -x "${VM_ID}" |tee "vm-${VM_ID}.xml" | "${_XPATH_A[@]}" "${_XPATH_QUERY[@]}" || true)
...
#    su - oneadmin -c "ssh $VMHOST 'virsh --connect $LIBVIRT_URI qemu-monitor-command \"$DEPLOY_ID\" --hmp \"block_resize $qemu_device ${DISK_SIZE}M\"'"
  su - oneadmin -c "ssh ${VMHOST} 'virsh --connect ${LIBVIRT_URI:-qemu:///system} blockresize \"${DEPLOY_ID}\" --path ${qemu_file} --size ${DISK_SIZE}M'"
  ret=$?
  if [[ "${ret}" -eq 0 ]]; then
    su - oneadmin -c "ssh ${VMHOST} 'virsh --connect ${LIBVIRT_URI:-qemu:///system} qemu-monitor-command \"${DEPLOY_ID}\" --hmp \"info block\"'"
    su - oneadmin -c "ssh ${VMHOST} 'virsh --connect ${LIBVIRT_URI:-qemu:///system} qemu-monitor-command \"${DEPLOY_ID}\" --hmp \"info blockstats\"'"
  else
