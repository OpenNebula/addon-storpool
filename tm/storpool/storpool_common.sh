...
function oneHostInfo()
{
    ...
    splog "oneHostInfo($_name): ID:$HOST_ID NAME:$HOST_NAME STATE:$HOST_STATE(${HostState[$HOST_STATE]}) HOSTNAME:${HOST_HOSTNAME}${HOST_SP_OURID:+ HOST_SP_OURID=$HOST_SP_OURID}"
}
...
oneVmVolumes()
{
    ...
    unset XPATH_ELEMENTS i
    while IFS= read -ru 5 -d '' element; do
        XPATH_ELEMENTS[i++]="${element}"
    done 5< <(cat "${VM_XML_FILE}" |\
        ${DRIVER_PATH}/../../datastore/xpath-sp.rb -s \
        /VM/STATE \
        /VM/LCM_STATE)
    unset i
